(function () {
    ...
    function renderSiteCard(site) {
        ...
        const canCheckIn = canCheckInAtSite(site);
        const uploadButtonText = canCheckIn ? "确认上传" : "进入打卡范围后可上传";
        const disabledButton = canCheckIn ? "" : " disabled";
        ...
        elements.siteCard.innerHTML = [
            ...
            "<section class=\"site-card-body\">",
                ...
                "<details class=\"upload-panel\">",
                    ...
                    "<p class=\"upload-copy\">到达地点周边后可提交现场照片与文字记录。</p>",
                    "<form id=\"checkinForm\" class=\"upload-fields\">",
                        ...
                        "<button class=\"upload-button\" type=\"submit\"", disabledButton, ">", escapeHtml(uploadButtonText), "</button>",
                    "</form>",
                "</details>",
            "</section>"
        ].join("");
        ...
        navigateButton.addEventListener("click", function () {
            window.open(buildNavigationUrl(site), "_blank");
        });
    ...
    async function submitCheckin(site, form) {
        ...
        if (!canCheckInAtSite(site)) {
            showToast("进入地点周边后方可提交");
            return;
        }
