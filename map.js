(function () {
    ...
    const BUILD_VERSION = "20260519-savedupload";
        ...
        elements.siteCard.innerHTML = [
            ...
            "<section class=\"site-card-body\">",
                ...
                "<details class=\"upload-panel\">",
                    ...
                    "<p class=\"upload-copy\">提交后会保存现场照片与文字记录，并同步显示在最近上传中。</p>",
    ...
    function createBackend(config) {
        ...
        if (mode === "local-api") {
            ...
            if (baseUrl) {
                return createLocalApiBackend({ baseUrl: baseUrl });
            }
        }
    ...
    function createSupabaseBackend(supabaseConfig) {
        ...
        async function uploadPhoto(record) {
            ...
            const objectPath = [storageFolder, sanitizePathSegment(record.siteId), record.id + "." + extension]
                .filter(Boolean)
                .join("/");
        ...
        return {
            ...
            createCheckin: async function (record) {
                ...
                return Object.assign({}, record, { photoUrl: photoUrl });
            }
        };
    }
